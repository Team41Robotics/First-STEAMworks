#include "LidarLite.h"
	...
	//returns in cm
	{
		...
		//read low byte
		Read(Commands::kLAST_DELAY_LOW,1,reinterpret_cast<uint8_t*>(&distance));
		distance >>= 8;	//shove low byte to lower 8

		//read high order
		Read(Commands::kLAST_DELAY_HIGH,1,reinterpret_cast<uint8_t*>(&distance));
	...
	{
		uint8_t status = 0;
		Read(Commands::kSTATUS,1,(&status));
		return status;
	}
	...
	{
		uint8_t config = 0;
		Read(Commands::kACQ_CONFIG_REG,1,(&config));
		return config;
	}
