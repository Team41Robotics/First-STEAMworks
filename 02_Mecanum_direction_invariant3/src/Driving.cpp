...
//    SmartDashboard::PutNumber("rotation",theta/RAD);
	...
	if(control_joy->GetRawButton(2)) //for direction invariant translation
	{
		...
		double joystickAngle = control_joy->GetDirectionRadians();//atan(con_x/(con_y*-1));
		double gyroRadians = (theta * (M_PI/180));
		double joystickMagnitude = sqrt((con_x*con_x)+(con_y*con_y));
		double VJoyX = sin(joystickAngle - gyroRadians) * joystickMagnitude;
		double VJoyY = cos(joystickAngle - gyroRadians) * joystickMagnitude * -1;
		...
		printf("joystick Angle: %f\n",joystickAngle*180/M_PI);
		printf("joystick Magnitude: %f\n",joystickMagnitude);
